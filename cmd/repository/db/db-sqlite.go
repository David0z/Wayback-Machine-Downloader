...
func (repo *SQLiteRepository) GetOne(websiteURL string, mimetypes []string) (*data.Link, error) {
	...
	query := fmt.Sprintf(`SELECT urlkey, timestamp, original, mimetype, statuscode, websiteurl, downloaded FROM links WHERE websiteurl = ? AND statuscode = '200' AND downloaded = false AND mimetype IN (%s) LIMIT 1`, placeholders)
